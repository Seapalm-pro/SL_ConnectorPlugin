...
import fr.mrbaguette07.slconnector.BridgedCommand;
import fr.mrbaguette07.slconnector.BridgedSuggestions;
import fr.mrbaguette07.slconnector.LocationInfo;
import fr.mrbaguette07.slconnector.ProxyBridgeCommon;
...
public class Bridge extends ProxyBridgeCommon<Bungeeslconnector, ProxiedPlayer> implements Listener {

    private Table<String, String, BridgedCommand<?, CommandSender>> commands = HashBasedTable.create();

    public Bridge(Bungeeslconnector plugin) {
        super(plugin);
        ...
        registerMessageHandler(Action.CONSOLE_COMMAND, (receiver, message) -> {
            ...
            boolean success = plugin.getProxy().getPluginManager().dispatchCommand(new BridgedSender(senderServer, id),
                    command);
        ...
        registerHandler(Action.EXECUTE_COMMAND, (receiver, data) -> {
            ...
            BridgedCommand<?, CommandSender> command = commands.get(pluginName.toLowerCase(Locale.ROOT),
            ...
            try {
                ...
                plugin.logError("Unhandled exception executing bridged command '" + commandLabel + "' from plugin "
                        + command.getPlugin().getName(), ex);
            }
        });
    }
    ...
    @Override
    public CompletableFuture<Boolean> teleport(ProxiedPlayer player, LocationInfo location,
    ...
    @Override
    public CompletableFuture<Boolean> teleport(ProxiedPlayer player, String serverName, String worldName,
            Consumer<String>... consumer) {
    ...
    public CompletableFuture<Boolean> teleport(String playerName, String serverName, String worldName,
        ...
        if (server == null) {
            ...
            future.complete(false);
            for (Consumer<String> c : consumer) {
                c.accept("Could not find server " + serverName + " on this proxy to teleport player " + player.getName()
                        + " to");
            }
            return future;
        }
    ...
    @Override
    public CompletableFuture<Boolean> teleport(ProxiedPlayer player, ProxiedPlayer target,
            Consumer<String>... consumer) {
        ...
        getServer(targetName).thenAccept(serverName -> {
            if (serverName == null || serverName.isEmpty()) {
                ...
                plugin.logDebug("Target player " + targetName
                        + " is either not online or not connected to a server. (Tried to teleport " + player.getName()
                        + " to them)");
                future.complete(false);
                for (Consumer<String> c : consumer) {
            ...
            if (server == null) {
                ...
                plugin.logDebug("Target player " + targetName + " is online on server " + serverName
                        + " which does not exist on this proxy! (Tried to teleport " + player.getName() + " to them)");
                future.complete(false);
                for (Consumer<String> c : consumer) {
                    c.accept("Could not find server " + serverName + " of target player " + targetName + " on "
                            + player.getName() + "'s proxy to teleport them");
                }
                return;
            }
    ...
    private void sendToServerIfNecessary(ProxiedPlayer player, ServerInfo server, CompletableFuture<Boolean> future,
            Consumer<String>... consumer) {
        if (!player.getServer().getInfo().equals(server) && plugin.getConnector().requiresPlayer()
    ...
    public void registerServerCommand(BridgedCommand<? extends Plugin, CommandSender> command) {
        commands.put(command.getPlugin().getName().toLowerCase(Locale.ROOT), command.getName().toLowerCase(Locale.ROOT),
                command);
        ...
        plugin.getProxy().getPluginManager().registerCommand(command.getPlugin(),
                new ForwardingCommand(command.getName(), command.getPermission(), command));
        for (String alias : command.getAliases()) {
            plugin.getProxy().getPluginManager().registerCommand(command.getPlugin(),
                    new ForwardingCommand(alias, command.getPermission(), command));
        }
    }
    ...
    private class ForwardingCommand extends Command implements TabExecutor {
        ...
        public ForwardingCommand(String name, String permission,
                BridgedCommand<? extends Plugin, CommandSender> command) {
        ...
        public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
            if (command instanceof BridgedSuggestions
                    && sender.hasPermission(command.getPermission() + ".tabcomplete." + getName())) {
