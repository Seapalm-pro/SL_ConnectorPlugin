...
public class TeleportCommand extends SubCommand {
    ...
    public boolean run(CommandSender sender, String[] args) {
        ...
        if (args.length == 2) {
            player.connect(server, (success, ex) -> {
                if (success) {
                    sender.sendMessage(ChatColor.GREEN + "Connected player " + player.getName() + " to server "
                            + server.getName());
                } else {
                    sender.sendMessage(ChatColor.RED + "Error while connecting player " + player.getName()
                            + " to server " + server.getName() + ": " + ex.getMessage());
                }
            });
            return true;
        }
    ...
    public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            return plugin.getProxy().getServers().values().stream().filter(s -> s.canAccess(sender))
                    .map(ServerInfo::getName).filter(s -> s.startsWith(args[0].toLowerCase(Locale.ROOT)))
                    .sorted(String::compareToIgnoreCase).collect(Collectors.toList());
        }
        return Collections.emptySet();
    }
