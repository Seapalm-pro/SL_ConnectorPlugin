...
public abstract class ProxyBridgeCommon<P extends slconnector<R>, R> extends BridgeCommon<P, R> {

    public ProxyBridgeCommon(P plugin) {
        super(plugin);

        registerMessageHandler(Action.STARTED,
                (receiver, message) -> registerServerCommands(message.getReceivedMessage().getSendingServer()));
