...
public abstract class Connector<P extends slconnector, R> {
    ...
    public Connector(P plugin, boolean requiresPlayer) {
        ...
        if (requiresPlayer) {
            plugin.logWarning(
                    "This messenger type requires at least one player connected to the sending and receiving server,"
    ...
    protected void handle(R receiver, Message message) {
        ...
        // If message group is empty then this should reach all, also targeting specific
        // servers/proxies is always allowed
        if (!message.getGroup().isEmpty() && message.getTarget() != MessageTarget.SERVER
                && message.getTarget() != MessageTarget.PROXY) {
            // Check group
            String group = plugin.getGroup(message.getSendingPlugin());
            if (!group.isEmpty() && !group.equals(message.getGroup())) {
                // Group is not empty and doesn't accept all messages and the message group
                // doesn't match the group
                return;
            }
        }

        BiConsumer<R, Message> handler = handlers.get(message.getSendingPlugin().toLowerCase(Locale.ROOT),
                message.getAction());
        if (handler != null) {
            handler.accept(receiver, message);
        } else {
            plugin.logDebug(
                    "Plugin '" + message.getSendingPlugin() + " did not register an action '" + message.getAction()
                            + "' but we received data with target '" + message.getTarget() + "' by " + receiver);
        }
    }
     ...
     * @param action The action for which data is sent
     * @param target Where to send data to
     * @param player Additional player data to use for sending (required in case the
     *               target is {@link MessageTarget#SERVER} or
     *               {@link MessageTarget#PROXY})
     * @param data   The data
     */
     ...
     * @param action     The action for which data is sent
     * @param target     Where to send data to
     * @param targetData Additional data to use for sending (required in case the
     *                   target is {@link MessageTarget#SERVER})
     * @param data       The data
     */
    ...
    public void sendDataInternal(ConnectingPlugin sender, String action, MessageTarget target, Object targetData,
            byte[] data) {
        if (target.getSource() != null && target.getSource() != plugin.getSourceType()) {
            throw new UnsupportedOperationException(
                    "Cannot send message with target " + target + " from " + plugin.getSourceType());
        }

        String group = plugin.getGroup(sender.getName());
        sendDataImplementation(targetData,
                new Message(group, target, plugin.getServerName(), sender.getName(), action, data));
    }
     ...
     *             {@link #registerMessageHandler(ConnectingPlugin, String, BiConsumer)}
     */
    @Deprecated
    public BiConsumer<R, byte[]> registerHandler(ConnectingPlugin plugin, String action,
            BiConsumer<R, byte[]> handler) {
        return transformHandler(
                registerMessageHandler(plugin, action, (r, message) -> handler.accept(r, message.getData())));
    }
     ...
     * @deprecated Since 1.5. Use
     *             {@link #unregisterMessageHandler(ConnectingPlugin, String)}
     */
    @Deprecated
     ...
     * @deprecated Since 1.5. Use
     *             {@link #unregisterMessageHandlers(ConnectingPlugin)}
     */
    @Deprecated
     ...
     * @since 1.5
     */
    public BiConsumer<R, Message> registerMessageHandler(ConnectingPlugin plugin, String action,
            BiConsumer<R, Message> handler) {
    ...
    /**
     * Whether this connector requires a player on the target server. (Mostly for
     * plugin message usage)
     * 
     * @return Whether this connector requires at least one player on the target
     *         server
     */
